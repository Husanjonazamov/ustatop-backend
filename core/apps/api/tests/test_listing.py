...
class ListingimageTest(TestCase):

    def _create_data(self):
        return ListingimageModel._create_fake()

    def setUp(self):
        self.client = APIClient()
        self.instance = self._create_data()
        self.urls = {
            "list": reverse("listingImage-list"),
            "retrieve": reverse("listingImage-detail", kwargs={"pk": self.instance.pk}),
            "retrieve-not-found": reverse("listingImage-detail", kwargs={"pk": 1000}),
        }
